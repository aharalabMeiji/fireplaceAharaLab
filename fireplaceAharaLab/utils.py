...
class Node(object):
    ...
    def expandChild(self, action):  # maya氏改訂版
        ...
        child = Node(self.expandingGame, action, self, getCandidates(
            self.expandingGame, _smartCombat=False))
        self.childNodes.append(child)
        return child
...
def play_set_of_games(P1: Agent, P2: Agent, gameNumber=15, debugLog=True):
    if debugLog:
        print(" %r (%s) vs.  %r (%s)" %
              (P1.name, P1.myClass, P2.name, P2.myClass))
    Count1 = 0
    Count2 = 0
...
def play_MechaHunterGames(P1: Agent, P2: Agent, gameNumber=15, debugLog=True):
    ...
    for i in range(gameNumber):
        winner = play_one_game(
            P1, P2, deck1=BigDeck.MechaHunter, deck2=BigDeck.MechaHunter, debugLog=debugLog)
        print("winner is %r" % winner)
        if winner == P1.name:
            Count1 += 1
        elif winner == P2.name:
                    ...
                    if card2.is_playable():
                        if card2.requires_target():
                            for target in card.targets:
                                myCandidate.append(
                                    Candidate(card, card2=card2, type=BlockType.PLAY, target=target))
                        else:
                            myCandidate.append(
                                Candidate(card, card2=card2, type=BlockType.PLAY, target=None))
            else:  # card2=None
                if card.requires_target():
                    for target in card.targets:
                        myCandidate.append(
                            Candidate(card, type=BlockType.PLAY, target=target))
                else:
                    myCandidate.append(
                        Candidate(card, type=BlockType.PLAY, target=None))
            ...
            for target in character.targets:
                if character.can_attack(target):
                    ...
                    if (myH > hisA) or (not _smartCombat):
                        myCandidate.append(
                            Candidate(character, type=BlockType.ATTACK, target=target))
    if player.hero.power.is_usable():
        if len(player.hero.power.targets) > 0:
            for target in player.hero.power.targets:
                myCandidate.append(
                    Candidate(player.hero.power, type=BlockType.POWER, target=target))
        else:
            myCandidate.append(Candidate(player.hero.power,
                                         type=BlockType.POWER, target=None))
    if _includeTurnEnd:
        # この選択肢は「何もしない」選択肢ですが、
        # ターンを終了することはできないので、
