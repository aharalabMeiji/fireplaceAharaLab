...
def printClasses():
	...
	print('')
	myCardSet=CardSet.ALTERAC_VALLEY##ALTERAC_VALLEY#THE_SUNKEN_CITY#REVENDRETH
	myCardClass=CardClass.HUNTER##DEMONHUNTER,DRUID,HUNTER,MAGE,NEUTRAL,PALADIN,PRIEST,ROGUE,SHAMAN,WARLOCK,WARRIOR
	mySetText="Alterac_"#Alterac_#Sunken_#Revendreth_#
	myClassText="Hunter"##DemonHunter,Druid,Hunter,Mage,Neutral,Paladin,Priest,Rogue,Shaman,Warlock,Warrior
	...
	for _id in db.keys():
		...
		if _card.card_set== myCardSet and _card.card_class == myCardClass: 
			print("%s%s=True"%(mySetText,_card.name.replace(' ','_').replace('-','_').replace("'",'').replace(':','')))
		pass
	pass
	#print(']')
	for _id in db.keys():
		_card = db[_id]
		if _card.card_set== myCardSet and _card.card_class == myCardClass: 
			print('if %s%s:# '%(mySetText,_card.name.replace(' ','_').replace('-','_').replace("'",'').replace(':','')))
