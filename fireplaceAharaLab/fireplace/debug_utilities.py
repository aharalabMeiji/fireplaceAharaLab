...
if Config.HEARTHSTONE==5:
	from .cards.cardlist import All


	def printClasses():
		myCardSet=CardSet.RETURN_OF_THE_LICH_KING
		#STORMWIND#ALTERAC_VALLEY#THE_SUNKEN_CITY#REVENDRETH#VANILLA
		myCardClass=CardClass.NEUTRAL
		##DEMONHUNTER,DRUID,HUNTER,MAGE,NEUTRAL,PALADIN,PRIEST,ROGUE,SHAMAN,WARLOCK,WARRIOR,DEATHKNIGHT
		setText={
		...
		for _id in db.keys():
			...
			if _card.card_set== myCardSet and _card.card_class == myCardClass: 
				if not keyID in _card.id:
					...
					f.write('\t\tself.con4=self.summon_card(self.controller, "minionH3")\n'%())
					f.write('\t\tself.opp1=self.summon_card(self.opponent, "minionH3")\n'%())
