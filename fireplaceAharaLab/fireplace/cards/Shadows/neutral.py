from ..utils import *

####### neutral-shadows,,46,,,,,,


class DAL_:
	"""Potion Vendor,,1,1,1,Minion,Common,-,Battlecry
	"""
class DAL_:
	"""Toxfin,,1,1,2,Minion,Common,Murloc,Battlecry
,,,,,,,,",?"
,,,,,,,,Poisonous
	"""
class DAL_:
	"""Arcane Servant,,2,2,3,Minion,Common,Elemental,-
	"""
class DAL_:
	"""Dalaran Librarian,,2,2,3,Minion,Common,-,Battlecry
,,,,,,,,",?"
,,,,,,,,Silence
	"""
class DAL_:
	"""EVIL Cable Rat,,2,1,1,Minion,Common,Beast,Battlecry
,,,,,,,,",?"
,,,,,,,,Lackey
	"""
class DAL_:
	"""Hench-Clan Hogsteed,,2,2,1,Minion,Common,Beast,Deathrattle
,,,,,,,,",?"
,,,,,,,,Rush
	"""
class DAL_:
	"""Mana Reservoir,,2,0,6,Minion,Common,Elemental,Spell Damage
	"""
class DAL_:
	"""Spellbook Binder,,2,3,2,Minion,Common,-,Battlecry
,,,,,,,,",?"
,,,,,,,,Spell Damage
	"""
class DAL_:
	"""Sunreaver Spy,,2,2,3,Minion,Common,-,Battlecry
,,,,,,,,",?"
,,,,,,,,Secret
	"""
class DAL_:
	""""Zayle, Shadow Cloak",,2,3,2,Minion,Legendary,-,-
	"""
class DAL_:
	"""Arcane Watcher,,3,5,6,Minion,Rare,-,Spell Damage
	"""
class DAL_:
	"""Faceless Rager,,3,5,1,Minion,Common,-,Battlecry
	"""
class DAL_:
	"""Flight Master,,3,3,4,Minion,Common,-,Battlecry
	"""
class DAL_:
	"""Hench-Clan Sneak,,3,3,3,Minion,Common,-,Stealth
	"""
class DAL_:
	"""Magic Carpet,,3,1,6,Minion,Epic,-,Rush
	"""
class DAL_:
	"""Spellward Jeweler,,3,3,4,Minion,Rare,-,Battlecry
	"""
class DAL_:
	"""Archmage Vargoth,,4,2,6,Minion,Legendary,-,-
	"""
class DAL_:
	"""Hecklebot,,4,3,8,Minion,Rare,Mech,Battlecry
,,,,,,,,",?"
,,,,,,,,Taunt
	"""
class DAL_:
	"""Hench-Clan Hag,,4,3,3,Minion,Epic,-,Battlecry
	"""
class DAL_:
	"""Portal Keeper,,4,5,2,Minion,Rare,Demon,Battlecry
,,,,,,,,",?"
,,,,,,,,Rush
	"""
class DAL_:
	"""Proud Defender,,4,2,6,Minion,Common,-,Taunt
	"""
class DAL_:
	"""Soldier of Fortune,,4,5,6,Minion,Common,Elemental,-
	"""
class DAL_:
	"""Traveling Healer,,4,3,2,Minion,Common,-,Battlecry
,,,,,,,,",?"
,,,,,,,,Divine Shield
	"""
class DAL_:
	"""Violet Spellsword,,4,1,6,Minion,Common,-,Battlecry
	"""
class DAL_:
	"""Azerite Elemental,,5,2,7,Minion,Epic,Elemental,Spell Damage
	"""
class DAL_:
	"""Barista Lynchen,,5,4,5,Minion,Legendary,-,Battlecry
	"""
class DAL_:
	"""Dalaran Crusader,,5,5,4,Minion,Common,-,Divine Shield
	"""
class DAL_:
	"""Recurring Villain,,5,3,6,Minion,Rare,-,Deathrattle
	"""
class DAL_:
	"""Sunreaver Warmage,,5,4,4,Minion,Rare,-,Battlecry
	"""
class DAL_:
	"""Eccentric Scribe,,6,6,4,Minion,Common,-,Deathrattle
	"""
class DAL_:
	"""Mad Summoner,,6,4,4,Minion,Rare,Demon,Battlecry
	"""
class DAL_:
	"""Portal Overfiend,,6,5,6,Minion,Epic,Demon,Battlecry
,,,,,,,,",?"
,,,,,,,,Rush
	"""
class DAL_:
	"""Safeguard,,6,4,5,Minion,Common,Mech,Deathrattle
,,,,,,,,",?"
,,,,,,,,Taunt
	"""
class DAL_:
	"""Unseen Saboteur,,6,5,6,Minion,Epic,-,Battlecry
	"""
class DAL_:
	"""Violet Warden,,6,4,7,Minion,Common,-,Spell Damage
,,,,,,,,",?"
,,,,,,,,Taunt
	"""
class DAL_:
	"""Chef Nomi,,7,6,6,Minion,Legendary,-,Battlecry
	"""
class DAL_:
	"""Exotic Mountseller,,7,5,8,Minion,Rare,-,-
	"""
class DAL_:
	"""Tunnel Blaster,,7,3,7,Minion,Rare,-,Deathrattle
,,,,,,,,",?"
,,,,,,,,Taunt
	"""
class DAL_:
	"""Underbelly Ooze,,7,3,5,Minion,Rare,-,-
	"""
class DAL_:
	"""Batterhead,,8,3,12,Minion,Epic,-,Rush
	"""
class DAL_:
	"""Heroic Innkeeper,,8,4,4,Minion,Common,-,Battlecry
,,,,,,,,",?"
,,,,,,,,Taunt
	"""
class DAL_:
	"""Jepetto Joybuzz,,8,6,6,Minion,Legendary,-,Battlecry
	"""
class DAL_:
	"""Whirlwind Tempest,,8,6,6,Minion,Epic,Elemental,Mega-Windfury
,,,,,,,,",?"
,,,,,,,,Windfury
	"""
class DAL_:
	"""Archivist Elysiana,,9,7,7,Minion,Legendary,-,Battlecry
,,,,,,,,",?"
,,,,,,,,Discover
	"""
class DAL_:
	"""Burly Shovelfist,,9,9,9,Minion,Common,-,Rush
	"""
class DAL_:
	"""Big Bad Archmage,,10,6,6,Minion,Epic,-,-
