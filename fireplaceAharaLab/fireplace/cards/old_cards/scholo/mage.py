...
class SCH_350:#OK
	"""Wand Thief	Common"""
	#[Combo:] [Discover] a Mage_spell.
	combo = DISCOVER(RandomSpell(card_class=CardClass.MAGE))

class SCH_353:#OK
	"""Cram Session	Rare"""
	#Draw $1 |4(card, cards) <i>(improved by [Spell Damage])</i>.
...
class SCH_241:#OK
	"""Firebrand	Common"""
	#[[Spellburst]:] Deal 4 damage randomly split among all_enemy minions.
...
class SCH_351:#OK
	...
	#[x][Battlecry:] Summon two random 5-Cost minions. Secretly pick one that dies _when it takes damage.
	import random
	def play(self):
