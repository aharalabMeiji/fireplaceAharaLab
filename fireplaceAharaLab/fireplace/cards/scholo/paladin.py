...
class SCH_250e:
	""" Apathetic """
	atk = SET(1)
	play = OWN_TURN_BEGIN.on(Destroy(SELF))
class SCH_149:
...
class SCH_149e:
	""" Best of the Best """
	atk = HIGHEST_ATK(ALL_MINIONS)
	health = HIGHEST_HEALTH(ALL_MINIONS)
class SCH_523:
...
class SCH_138:
	...
	play = Buff(Buff(SELF, "SCH_138e"), "SCH_138e2")
class SCH_138e:
	atk=8
	health=8
class SCH_138e2:
...
class SCH_533:
	...
	play = Summon(CONTROLLER, SetTag(RANDOM(FRIENDLY_DECK), (GameTag.TAUNT, GameTag.DIVINE_SHIELD)))
class SCH_135:
	...
	events = Attack(SELF, ENEMY_MINIONS).after(Buff(Attack.DEFENDER, "SCH_135e"))
class SCH_135e:
	""" Schooled """
	atk = SET(3)
	health = SET(3)
